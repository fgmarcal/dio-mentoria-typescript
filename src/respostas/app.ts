...
let apiKey:string;
let requestToken:string | Promise<string>;
let username:string;
let password:string;
...
searchButton.addEventListener('click', async () => {
    ...
    let listaDeFilmes = await procurarFilme(queryVal);
...
class HttpClient {
    static async get({url = "", method = "", body = ""}) : Promise<string> {
...
async function adicionarFilmeNaLista({ filmeId, listaId }: { filmeId: string; listaId: string; }): Promise<void> {
    let result = await HttpClient.get({
